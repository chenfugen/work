<template>
	<div class="layout">
		<Layout class="layout-main">
			<!-- 侧边导航栏 -->
			<Sider v-show="showSider" class="layout-sider" collapsible ref="side" hide-trigger :collapsed-width='0'>
				<div class="logoHome">
					<img :src="logoImg" alt="logo">
				</div>
				<Menu accordion v-if='!collapsed' :open-names="open_menu" :active-name='active_menu' theme="dark" width="auto">
			...
			<Layout :class="layoutClass">
					...
					<Layout>
						<Content class="layout-header-right">
							...
							<div class="layout-header-top clearfloat">
								<Icon class="headerIcon" v-show='menu.length > 0' @click.native="collapsedSider" :class="rotateIcon" :style="collapsedStyle"
								 type="md-menu" size="30"></Icon>
								<Dropdown class="myMenu" trigger="click" style="margin-left: 7px" @on-click="handleChangeModel">
									<a href="javascript:void(0)" class="mySelectMenu">
										{{model=='manageSystem'?'管理中心':model=='dataSystem'?'数据中心':'产测系统'}}
									</a>
									<DropdownMenu slot="list" v-if="showDropdownMenu">
										<DropdownItem v-for='(item, index) in permission' :key='index' :name="item.name">{{item.title}}</DropdownItem>
									</DropdownMenu>
								</Dropdown>
				...
				<Content class="layout_content">
					...
					<div v-if='showSwitch==1' class="right" style="margin-top:5px;">
				...
				<div class="layout_footer">
					©2020 CANATURE canature.com开能健康集团  技术支持：海大物联
				</div>
			</Layout>
		</Layout>
	...
	export default {
		...
		created() {
			...
			if (this.$store.state.user.customerId != "") {
				...
				if(sessionStorage.getItem("webLogoImage")){
					this.logoImg = sessionStorage.getItem("webLogoImage")
				}		
			}else{
				 this.logoImg = require('../../assets/logoHome.png')
			}
		},
		computed: {
			showSwitch() {
			...
			handleChangeModel(modelName) {
				...
				for (var i = 0; i < this.permission.length; i++) {
					if (this.permission[i].name === modelName) {
						if (this.permission[i].child && this.permission[i].child.length > 0) {
							this.layoutClass = 'openLayout'
							this.showSider = true
							this.isLeftClass = false
							this.$router.push({
				...
				switch (name) {
					...
					let para={
						...
						this.$Modal.confirm({
							...
							cancelText: '取消',
							onOk: () => {
								this.$store.dispatch('handleLogout',para) // 重置菜单
							}
						})
				}
			},

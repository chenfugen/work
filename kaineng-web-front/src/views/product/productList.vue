<template>
	<div class="product">
		...
		<!-- 详情表单 -->
		<Modal v-model="detailModal" title="产品详情" class-name="detailModal" :mask-closable="false" :width="700" :footer-hide="true"
		...
		<!-- 委托商绑定 -->
		<Modal v-model="dialogAddPartner" title="绑定委托商" class-name="detailModal" :mask-closable="false" :width="500"
		 :footer-hide="true" @on-cancel="cancel">
			<Form ref="formAddPartner" :model="formAddPartner" :rules="ruleformPartner" :label-width="100">
				<FormItem prop="partnerId">
					<Select v-model="formAddPartner.partnerId" style="width:260px" placeholder="选择绑定的委托商">
						<Option v-for="item in partnerList" :key="item.partnerId" :value="item.partnerId">{{ item.partnerName }}</Option>
					</Select>
				</FormItem>
				<FormItem>
					<Button type="primary" @click="handleConfirm('formAddPartner')">确认</Button>
					<Button @click="cancel" style="margin-left: 8px">取消</Button>
				</FormItem>
			</Form>
		</Modal>
		<!-- 编辑表单 -->
		<Modal v-model="editModal" title="产品编辑" class-name="editModal" :mask-closable="false" :width="700" @on-visible-change="closeModal"
		 @on-ok="handleSubmit" @on-cancel="cancelEdit">
			<div class="detailWrap">
					...
					<Tabs :value="'netImg'">
						<TabPane label="配网/绑定说明" name="netImg">
							<div class="phone">
								...
								<div class="uploadWrap">
									...
									<Upload class="uploadDiv" :action="uploadUrl" :data="upLoadData" :show-upload-list="false" :format="['jpg' ,'jpeg' ,'png', 'gif', 'bmp']"
									 accept="image/jpg,image/jpeg,image/png,image/gif,image/bmp" :on-success="uploadNetSuccess" :on-error="uploadError">
										<Button>更改图片</Button>
									</Upload>
								</div>
								<div class="uploadWrap">
									<!-- eslint-disable-next-line -->
									<Upload class="uploadDiv" :action="uploadUrl" :data="upLoadData" :before-upload='uploadBefore' :max-size='1000'
									 :show-upload-list="false" :format="['jpg','jpeg','png', 'gif', 'bmp']" accept="image/jpg,image/jpeg,image/png,image/gif,image/bmp"
						...
						<TabPane label="产品图片" name="productImg">
							<div class="phone">
								...
								<div class="uploadWrap">
									<!-- eslint-disable-next-line -->
									<Upload class="uploadDiv" :action="uploadUrl" :data="upLoadData" :before-upload='uploadBefore'
									 :show-upload-list="false" :format="['jpg','jpeg','png', 'gif', 'bmp']" accept="image/jpg,image/jpeg,image/png,image/gif,image/bmp"
									 :on-success="uploadProductSuccess" :on-error="uploadError">
	...
	export default {
		...
		data() {
			return {
				...
				dialogAddPartner: false, // 绑定委托商弹窗
				partnerList:JSON.parse(sessionStorage.getItem("partners")), //委托商列表
				editProduct:'', //绑定委托商的PK产品
				formAddPartner: {
					partnerId: ''
				},
				ruleformPartner: {
						...
						render: (h, params) => {
							return h('div', [
								...
								h('Button', {
									...
									style: {
										marginRight: '5px',
										display:this.$store.state.user.userName=='root'?"inline-block":"none"
									},
									on: {
										click: () => {
								...
								}, params.row.customerName != null ? '解绑委托商' : "绑定委托商"),
							]);
						}
					}
				],
		...
		methods: {
			...
			handleConfirm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$Ax.post(api, {
							partnerId: this.formAddPartner.partnerId,
							productKey:this.editProduct,
						}).then(res => {
							if (res.success) {
								this.cancel();
			...
			handleBind(row, type) {
				...
				if (row.customerName==null) {
					this.dialogAddPartner = true
					this.formAddPartner.partnerId=null
					this.editProduct=row.productKey
				} else {
					activeName = row.productKey
					...
					this.$Modal.confirm({
						title: '提示',
						content: '<p>确定要将该' + activeName + '产品与委托商' + row.customerName + '解绑</p>',
						onOk: () => {
							this.$Ax.post(api, para).then(res => {
								if (res.success) {

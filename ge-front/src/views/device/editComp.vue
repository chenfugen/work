    ...
    <Form v-model='form' :label-width="80">
        ...
        <FormItem label='设置值：'>
            ...
            <Select v-if='itemData.dataType=="bool"' v-model="form.value">
                <Option v-for='(item,index) in dateRange' :value='item.value' :key='index' :label='item.label'></Option>
            </Select>
            <!-- array -->
            <div v-if='itemData.dataType=="array"' class="">
                ...
                <div class="" v-for='(item,index) in arrList'>
                    <InputNumber class="input" v-model="item.value"></InputNumber>
		...
		itemData: {
			...
			handler: function (val) {
				...
				if (val.dataType == "bool") {
					for (let i in val.dataRange) {
						list.push({
							value: Number(i),
							label: val.dataRange[i],
						});
					}
				}
				this.$set(this, "dateRange", list);
			},
		},
	},
	methods: {
		...
		setItemData(data) {
			...
			if (
				...
				for (let item in data.dataRange) {
					if (
						data.dataRange[item].dataType == "int" ||
						data.dataRange[item].dataType == "double" ||
						data.dataRange[item].dataType == "float"
					) {
						data.dataRange[item].dataRange.max = Number(
							data.dataRange[item].dataRange.max
						);
						data.dataRange[item].dataRange.min = Number(
							data.dataRange[item].dataRange.min
						);
						data.dataRange[item].dataRange.step = Number(
							data.dataRange[item].dataRange.step
						);
			...
			if (this.itemData.dataType == "bool") {
				this.form.valueNow = this.itemData.dataRange[
					data.value.toString()
				];
			} else {

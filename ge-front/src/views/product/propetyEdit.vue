...
<div class="propetyEdit" >
    <Modal v-model="editPropModal" width='90' title="分配产品属性" >
        <div slot='header' class='modalHeader'>
            ...
            <Button
				type="primary"
				class="product-manage-detail-btn"
			...
			>
				导入产品属性
				<input
					@input="handleImportClick"
					class="product-manage-detail-upload"
	...
	methods: {
		...
		handleSearchPropety() {
			...
			for (let i in this.propertyList) {
				...
				if (
					...
					this.propertyList[i].dataType != this.filter.dataType
				) {
					this.propertyList[i].disShow = true;
				}
				if (
					this.filter.propertyName != "" &&
					this.propertyList[i].propertyName.indexOf(
						this.filter.propertyName
					) == -1
				) {
					this.propertyList[i].disShow = true;
		...
		handleSubmitAdd() {
			...
			if (list == "") {
				...
				this.$Ax
					.post(this.$api.manage_product_update_property, para)
					.then((res) => {
						this.handleSubmitSub();
					})
					.catch((err) => {

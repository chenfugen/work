			...
			for (let i in list) {
				...
				this.markers.push(
					new AMap.Marker({
						...
						content:
							'<div style="background-color: #fff; height: 24px;text-align:center; width: 24px; border: 1px solid hsl(180, 100%, 40%); border-radius: 12px; box-shadow: hsl(180, 100%, 50%) 0px 0px 1px;">' +
							num +
							"</div>",
		...
		handleMakerCluster() {
			...
			var _renderClusterMarker = function (context) {
				...
				for (let i in context.markers) {
					nums = nums + context.markers[i].w.nums;
				}
				if (that.type == 2) {

			...
			<div style="padding-right:40px;">
				...
				>
					...
					<el-form-item label="固件类别" prop="type">
						<el-select size="small" placeholder="请选择固件类别" v-model.trim="ruleForm.type">
							<el-option
								v-for="item in $staticData.moreFirmwareTypes"
								:key="item.value"
								:label="item.label"
								:value="item.value"
							></el-option>
...
export default {
	...
	mounted() {
		this.getMoreFirmware();
	},
	methods: {
		// 获取表格数据
		getMoreFirmware() {
			this.$http.moreFirmwares(this.MCU).then((res) => {
				if (res.data.success) {
		...
		submitForm(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					...
					var ajax = this.$http.addMoreFirmware;
					if (!this.isAdd) {
						ajax = this.$http.updataMoreFirmware;
					}
					if (!this.choseFiles) {
						ajax(obj).then((res) => {
							this.addBox = false;
		...
		deleteRow(index, row) {
			this.$confirm("确认删除此版本吗?", "提示", this.$filters.confirm())
				.then(() => {
					this.$http.deleteMoreFirmware(row.id).then((res) => {
						if (res.data.success) {
							this.getMoreFirmware();
						}
					});

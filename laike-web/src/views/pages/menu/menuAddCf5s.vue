<template>
	<div id="menuAdd" class="menuAddCf5s">
		<el-form
			:model="ruleForm"
			:rules="rules"
			ref="ruleForm"
					...
					<div class="basic-left" style="width: 350px; float: left">
						...
						<el-form-item label="菜谱ID:" prop="menuId" ref="menuId">
							<el-input
								type="number"
								@blur="idBlur"
								:onblur="handleMenuId('blur')"
								:onch="handleMenuId('ch')"
						...
						<el-form-item label="份量:" prop="people" ref="people" :rules="peopleRules">
							<el-input size="small" v-model.trim="ruleForm.people" :maxlength="20"></el-input>
						</el-form-item>

						<el-form-item label="烹饪耗时:" prop="minute" ref="minute" :rules="minuteRules">
							<el-input-number
								size="mini"
								step-strictly
					...
					<div class="steps-conter">
						...
						<div class="steps-c-l">
							...
							<el-form-item label="档位设置:" prop="gear" v-if="item.clickType == 1" ref="gear">
								...
								>
									<el-option
										v-for="item in $staticData.menuGearList"
										:key="item.value"
										:label="item.label"
										:value="item.value"
									></el-option>
						...
						<div class="steps-c-r">
							...
							>
								...
								>
									<el-option
                                        v-for="item in $staticData.menuTempListCF5"
                                        :key="item"
                                        
                                     label="不设置(加热不启动)" :value="0">不设置(加热不启动)</el-option>
...
export default {
	...
	methods: {
		...
		handleMenuId: function (type) {
			if (type == "blur") {
				return this.ruleForm.menuId == "0000"
					? (this.ruleForm.menuId = "0001")
					: this.idQ == 3 && this.ruleForm.menuId == "000"
					? (this.ruleForm.menuId = "001")
					: "";
			} else {
				return this.ruleForm.menuId.length >= this.idQ
					? (this.ruleForm.menuId = this.ruleForm.menuId.slice(
							0,
							this.idQ
					  ))
					: "";

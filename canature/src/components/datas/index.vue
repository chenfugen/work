	...
	<v-touch v-on:swipeleft="swiperleft" :swipe-options="{direction: 'horizontal'}" v-on:swiperight="swiperright" class="wrapper">
		<div class="cityPara">
			<div class="areaPara">
				...
				<p class="weatherMsg"><span>{{weather.weather}}</span><span>空气质量   {{weather.humidity | airFilter}}</span></p>
			</div>
			<p class="changeCity" @click="searchCity">
				<i class="iconfont iconditu mapIcon"></i>
				<span class="cityName">{{positionMsg.city}}</span>
				<van-icon name="arrow" class="link" />
				<span class="cityTitle">更换城市</span>
			</p>
			<div class="water">
				<p class="cityWaters">{{totalWaters}}L</p>
				<p class="waterTitle"><i class="iconfont iconliuliang" style="color: #267CFB;"></i> 本年总用水量</p>
			</div>
			<div class="chart" v-show="showChart">
				<p class="chartName">家庭用水统计(L)</p>
				<ul class="van_tab">
	...
	.cityPara {
		...
		.changeCity {
			...
			.link {
				...
				font-size: 0.14rem;
				color: #969799;
				margin: 0.14rem 0.02rem 0px 0px;
			}
		}
		.water {

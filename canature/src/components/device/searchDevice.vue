<template>
	<div id="searchDevice">
		...
		<div v-show="!isProductMsg" class="scanCode">
			<van-button round class="addDevice" @click="scan" :disabled="isDisable">开始扫码</van-button>
		</div>
	</div>
</template>
								...
								success: function(res) {
									wx.scanQRCode({
										...
										success: function(res) {
											...
											if(result.split("?")[0] != that.$store.state.url + "login.html" && result.split("?")[0] !="https://smart.canature.com/login.html") {
												that.$toast("二维码地址错误");
												return false;
											}
											...
											that.$Ax.get("wechat/device/register/checkPK", {
												...
												if(res.data.success) {
													...
													window.location.href = "https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=MjM5MjE2OTk3Mg==&scene=123&from=singlemessage&isappinstalled=0#wechat_redirect";
												}
											})
										}
									});
...
<style lang="scss">
	#searchDevice {
		...
		.addImg {
			width: 40%;
			height: 3.1rem;
			background: url(../../../static/image/netWork2@3x.png) no-repeat;

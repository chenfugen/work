  ...
  <v-touch v-on:swipeleft="swiperleft" :swipe-options="{direction: 'horizontal'}" v-on:swiperight="swiperright" class="wrapper">
    <div class="commercialDevice" :class="{'normal':isnormal==1,'fault':isnormal==2,'offline':isnormal==0}">
      ...
      <ul class="waterMsg" :class="{'normalBg':isnormal==1,'faultBg':isnormal==2,'offlineBg':isnormal==0}">
        ...
        <li>
          <p class="waterValue" v-if="deviceMsg.heating.value==0"><img src="../../../../static/image/icon_heating_dis.png"
        ...
        <li>
          <p class="waterValue" v-if="deviceMsg.heating.value==1"><img src="../../../../static/image/icon_energysaving_dis.png"
              onclick="return false" /></p>
          <p class="waterValue" v-else><img src="../../../../static/image/btn_refrigeration_dis.png" onclick="return false" /></p>
          <p class="waterSet">节能</p>
        </li>
        <li>
          <p class="waterValue" v-if="deviceMsg.childLock.value==0"><img src="../../../../static/image/icon_child lock_dis.png"
              onclick="return false" /></p>
          <p class="waterValue" v-else><img src="../../../../static/image/icon_child lock.png" onclick="return false" /></p>
          <p class="waterSet">童锁</p>
        </li>
        <li>
          <p class="waterValue" v-if="deviceMsg.sterilizBtn.value==0"><img src="../../../../static/image/icon_uv_dis.png"
              onclick="return false" /></p>
          <p class="waterValue" v-else><img src="../../../../static/image/icon_uv.png" onclick="return false" /></p>
          <p class="waterSet">杀菌</p>
        </li>
      </ul>
      <Myecharts :device="device" idName="newCommercialOne" ref="init"></Myecharts>
    ...
    methods: {
      ...
      init() {
        this.$Axios.get("wechat/device/deviceDetail?deviceId=" + this.$route.params.id).then((res) => {
          if (res.data.success) {
            ...
            document.title = this.device.deviceNickName == this.device.deviceName ? this.device.productName : this.device
              .deviceNickName;
            if (this.isHint) {
              this.$nextTick(
                function() {
      ...
      set(e) {
        ...
        switch (e) {
          ...
          case 7:
            location.href =
              "https://wx.canature.com/crm2prod/api/weixinsso/authorize?homeurl=/serveHome/intelligence/" + this.$cookies
              .get("openId") + "/" + this.device.sncode;
            break;
          case 8:
            if (this.$route.params.bindType == 2) {
              this.$toast("您是该设备分享用户，无法分享")

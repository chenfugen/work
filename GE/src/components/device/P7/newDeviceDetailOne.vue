  ...
  <v-touch v-on:swipeleft="swiperleft" :swipe-options="{direction: 'horizontal'}" v-on:swiperight="swiperright" class="wrapper">
    <div class="p7newdevice">
      ...
      <div class="flow">
        ...
        <van-circle v-model="currentRate" :rate="deviceMsg.tdsOut.value/2" size="100" :stroke-width="50" color="#00B5E2 "
          layer-color="none" :speed="10" class="main" />
      </div>
      ...
      <Myecharts v-show="!isShowDate" :device="device" idName="P7newchartOne" ref="init"></Myecharts>
      <ul class="waterMsg" v-show="isShowDate">
        <li>
          <p class="waterSet">PCF值(%)</p>
          <p class="waterValue" :class="{isDanger:deviceMsg.filterPCF.value<11}">{{deviceMsg.filterPCF.value}}</p>
          <div class="line"></div>
        </li>
        <li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goMore() {
        let urlName = "/more?deviceId=" + this.$route.params.id + "&bindType=" + this.$route.params.bindType +

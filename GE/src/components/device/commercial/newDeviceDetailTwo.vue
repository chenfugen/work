<template>
	<v-touch v-on:swipeleft="swiperleft" :swipe-options="{direction: 'horizontal'}" v-on:swiperight="swiperright" class="wrapper">
		<div class="commercialNewDevice">
			...
			<div class="flow" @click="set(1)">
				<p class="waterNum">{{deviceMsg.tdsOut.value}}<span v-if="deviceMsg.tdsOut.value>10 && deviceMsg.tdsOut.value<120" class="status">优</span>
				 	<span v-else class="status">良</span>
				 </p>
			...
			<Myecharts v-show="!isShowDate" :device="device" idName="newCommercialTwoChart" ref="init"></Myecharts>

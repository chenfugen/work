	...
	<v-touch v-on:swipeleft="swiperleft" :swipe-options="{direction: 'horizontal'}" v-on:swiperight="swiperright" class="wrapper">
		<div class="allInone" :class="{'normal':isnormal==1,'fault':isnormal==2,'offline':isnormal==0}">
			<div class="searchIcon">
				...
				<img v-if="bindType!=2" src="../../../../static/image/icon_qrcode@2x.png" class="qrcodeIcon" @click="set(7)" />
			</div>
			<ul class="tdsContent">
				<img :src="tdsImg" />
			...
			<div class="waterMsg" :class="{'normalBg':isnormal==1,'faultBg':isnormal==2,'offlineBg':isnormal==0}">
				...
				<Myecharts :device="device" idName="allInTwoChart" ref="init"></Myecharts>
			</div>
			...
			<van-popup v-model="more" position="bottom">
				<ul class="oprateList">
					...
					<li class="oprateWay" v-show="isnormal==2" @click="set(6)">
			...
			init() {
				this.$Axios.get("wechat/device/deviceDetail?deviceId=" + this.$route.params.id).then((res) => {
					if(res.data.success) {
						...
						if(this.deviceMsg.material2.value.remain < 10 || this.deviceMsg.material1.value.remain < 10) {
			...
			set(e) {
				...
				switch(e) {
					...
					case 4:
						...
						break;
					case 5:
						urlName = "/productState";
						break;
					case 6:
						location.href = "http://wx.canature.com/crm2test/api/weixinsso/authorize?homeurl=/serveHome/intelligence/" + this.$cookies.get("openId") + "/" + this.device.sncode;
						break;
					case 7:

...
<script>
	...
	export default {
		...
		watch: {
			'$route' (to, from) {
				if(sessionStorage.getItem("fromPath") == to.path) {
					this.transitionName = 'slide-right';
				} else {
					this.transitionName = 'slide-left';
				}
				const arr = ['/device', '/datas', '/map', '/user'];
				if(arr.indexOf(to.path) >= 0 && arr.indexOf(from.path) >= 0) {
					const compare = arr.indexOf(from.path) > arr.indexOf(to.path);
		...
		methods: {
			judgeBind() {
				let that = this;
				that.$Ax.get("wechat/device/register/getDeviceBindStatus?productKey=" + that.$cookies.get('productMsg').productKey + "&deviceName=" + that.$cookies.get('productMsg').DeviceName + "&accessKey=" + that.$store.state.accessKey).then((res) => {
					if(res.data.success) {
						if(res.data.data.bindStatus) {
							let useId = res.data.data.userId;
							if(res.data.data.userId == that.$cookies.get('userId')) {
								that.$router.push({
									path: func.getRouterPath(that.$cookies.get('productMsg').productKey, res.data.data.deviceId, 0, 1)
								});
								that.$cookies.remove('productMsg');
								return false;
							}
							if(that.$cookies.get('Authorization') != null) {
								that.$Axios.get("wechat/device/getDeviceShareStatus?productKey=" + that.$cookies.get('productMsg').productKey + "&deviceName=" + that.$cookies.get('productMsg').DeviceName).then((res) => {
									if(res.data.data.shareStatus) {
										that.$router.push({
											path: func.getRouterPath(that.$cookies.get('productMsg').productKey, res.data.data.deviceId, 0, 2)
										});
										that.$cookies.remove('productMsg');
									} else {
										that.$dialog.confirm({
											...
											that.client.publish('/' + that.productKey + '/' + that.DeviceName + '/user/message/update', JSON.stringify(data), {
												qos: 1
											});
											that.$cookies.remove('productMsg');
										}).catch(() => {
			...
			mqtt() {
				...
				var options = {
					...
					password: CryptoJS.HmacSHA1("clientId" + clientId + "deviceName" + that.DeviceName + "productKey" + that.productKey + "timestamp" + that.timestamp, that.DeviceSecret).toString(),
					rejectUnauthorized: false
				}
				that.client = mqtt.connect("wss://" + that.productKey + ".iot-as-mqtt.cn-shanghai.aliyuncs.com:443/Mosca", options);
				...
				that.client.on('message', function(topic, message, packet) {
					...
					if(information.cmd == "share") {
						that.$dialog.confirm({
							title: '设备分享请求',
							message: '<img src='+information.data.headUrl+' class="headImg" />用户请求分享' + information.data.deviceName + '设备',

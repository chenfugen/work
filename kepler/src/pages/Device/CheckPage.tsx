...
import style from './css/CheckPage.module.css'
import api from '../../api'
...
const Empty = () => {
  return (
    <div className={classnames([style.container, style.background])}>
      ...
      <span>您当前未关联到任何型号，无法查看设备信息。</span>
      <span>请联系管理员进行关联；或先新增型号进行关联，再添加设备查看。</span>
    </div>
  )
}
...
const CheckPage = () => {
  ...
  return <>{loading ? <Loading /> : hasModel ? <DevicePage /> : <Empty />}</>
}

	...
	<div class="roleManage">
		...
		<div class="content">
			...
			<search :searchList="roleSearchList" :searchParams="roleSearchParams" formName="roleSearch" @resetForm="resetForm"
			 @submitForm="submitForm" :querySearch="querySearch" isAdd buttonName="新建角色" @add="addRole"></search>
		...
		<el-dialog title="权限查看" width="30%" :visible.sync="dialogpermissionVisible" :class="{ searchTree: dialogpermissionVisible }">
			<el-tree :props="defaultProps" :default-expanded-keys="['数码管理', '设备管理', '用户管理','配置中心','客户服务','系统管理']" node-key="moduleName"
			 ref="searchTree" :data="permission" show-checkbox></el-tree>
		</el-dialog>
	</div>
</template>
	...
	export default {
						...
						render: (h, params) => {
							return h('div', [
								h('el-link', {
									props: {
										type: 'primary',
										disabled: params.row.roleName == 'root' ? true : false,
									},
									on: {
										click: () => {
											this.end(params.row)
		...
		methods: {
			querySearch(queryString, cb) {
				let restaurants = this.roleListAuto;
			...
			getRoles() {
				...
				this.$Ax.get(api, {
					...
					if (res.success) {
						...
						this.roleListAuto = roleListAuto;
					} else {
						this.$message({
							message: res.message,
							type: 'warning'
				...
				this.$Ax.get(api, {
					...
					if (res.success) {
						...
						for (let j = 0; j < this.permission.length; j++) {
							this.permission.sort((a, b) => {
								return a.sort - b.sort
							})
							if (this.permission[j].child != null) {
								for (let i = 0; i < this.permission[j].child.length; i++) {

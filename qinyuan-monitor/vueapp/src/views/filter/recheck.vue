	...
	<div class="taskRemark">
		...
		<div class="content">
			...
			<div class="scanResult">
				<div v-if="!showResult">
					...
					<p class="scanHint">暂无数据，请扫描数码开始复检</p>
				</div>
				<div v-else>
		...
		<div class="content">
			...
			<search :searchList="searchList" :searchParams="searchParams" formName="searchParams" @resetForm="resetForm"
			 @submitForm="submitForm" isAdd buttonName="继续生产" @add="add"></search>
			<table-page :tableDable="orderLog" :page="page" :limit="limit" :columns="logcolumns"></table-page>
	...
	export default {
		data() {
			return {
				...
				orderDetail: {},
				finishedTotal: sessionStorage.getItem("remarkNum") == null ? 0 : sessionStorage.getItem("remarkNum"),
				deviceId: "",
				tone: "",
				searchList: [{
					label: "滤芯数码",
				...
				logcolumns: [{
					...
					},
					{
						...
						align: 'left',
						render: (h, params) => {
							return h('span', this.formatDate(params.row.recheckTime))
						}
					},
					{
						prop: 'recheckStatus',
						...
						render: (h, params) => {
							return h('el-tag', {
								props: {
									size: 'mini',
									effect: 'dark',
									type: params.row.recheckStatus == 1 ? 'success' : 'danger'
								} // 组件的props
							}, params.row.recheckStatus == 1 ? '成功' : '失败')
						}
					}
				],
				tableDable: [],
		...
		methods: {
			...
			checkTask(filterCode) {
				let api = this.$Api.manage_recheck;
				this.$Axios.post(api, {
					...
					this.finishedTotal++;
					sessionstorage.setItem("remarkNum", this.finishedTotal);
				}).catch((res) => {});
			},
			getOrderDetail() {
			...
			getCheckRecordPageList() {
				let api = this.$Api.manage_getRecheckRecordPageList;
				this.$Axios.get(api, {
					params: {
						pageNum: this.page,
						pageSize: this.limit,

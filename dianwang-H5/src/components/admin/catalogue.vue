<template>
	<div class="catalogue">
		...
		<ul class="searchList">
			<li v-for="(item,index) in catalogue" :key="index" @click="skip(item.directoryId,item.model)">
				<div class="left">
					<p class="name">{{item.model}}</p>
					<p class="detail"><span>资产: {{item.assetNums}} {{item.unit | nullDeal}}</span>&nbsp;&nbsp;<span>在用:
							{{item.usingNums}} {{item.unit | nullDeal}}</span></p>
				</div>
				<div class="right">
		...
		<div class="confirm_button">
		   <van-button size="large" color="#229794" class="createCatalogue" @click="createCatalogue(catalogueId)">新建资产名录</van-button>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			skip(id, model) {
				this.$router.push("/device/" + id + "/" + model + "/" + this.catalogueName + "/" + this.$route.params.assetName);
			},
			search() {
				this.$router.push("/search");
			},
			createCatalogue(id) {
				this.$router.push("/createCatalogue/" + id + "/" + this.catalogueName + "/" + this.$route.params.assetName)
			}
		}
	}
</script>
